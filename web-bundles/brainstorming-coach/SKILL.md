...
This web-bundle build offers only the Facilitator stance described here — the protocol above never generates ideas, so there is no Creative Partner mode. It also has no "Ideate for me" autonomous mode, since it does not ship the native skill's on-disk `memlog.py` workflow.
