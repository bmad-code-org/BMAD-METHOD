...
class InstallModeDetector {
  /**
   * Detect install mode based on manifest
   * @param {string} projectDir - Project directory
   * @param {string} currentVersion - Current/new version to install
   * @returns {string} Mode: 'fresh', 'update', 'reinstall', or 'invalid'
   */
  detectInstallMode(projectDir, currentVersion) {
    const manifestPath = this.getManifestPath(projectDir);

    // Check if manifest exists
    if (!fs.existsSync(manifestPath)) {
      console.log(`[Detector] Fresh install: No manifest found at ${manifestPath}`);
      return 'fresh';
    }
    ...
    try {
      ...
      if (!manifest || !manifest.version) {
        console.log('[Detector] Invalid manifest: Missing version field');
        return 'invalid';
      }
      ...
      // Compare versions
      const comparison = this.compareVersions(installedVersion, currentVersion);

      if (comparison === 0) {
        // Same version
        console.log(`[Detector] Reinstall mode: Same version (${installedVersion})`);
        return 'reinstall';
      } else if (comparison < 0) {
        // Installed version is older than current
        console.log(`[Detector] Update mode: ${installedVersion} → ${currentVersion}`);
        return 'update';
      } else {
        // Installed version is newer (downgrade scenario)
        console.log(`[Detector] Update mode: ${installedVersion} → ${currentVersion} (downgrade)`);
        return 'update';
      }
    } catch (error) {
      if (error instanceof yaml.YAMLException) {
        console.log('[Detector] Invalid manifest: YAML parsing error');
        return 'invalid';
      }
      console.log(`[Detector] Invalid manifest: ${error.message}`);
      return 'invalid';
    }
  }
   ...
   * @param {string} v1 - First version (e.g., "4.36.2")
   * @param {string} v2 - Second version (e.g., "4.39.2")
  ...
  compareVersions(v1, v2) {
    ...
    // If pre-release versions differ, handle them
    if (v1.includes('-') && !v2.includes('-')) return -1; // v1 is pre-release
    if (!v1.includes('-') && v2.includes('-')) return 1; // v2 is pre-release
    if (v1.includes('-') && v2.includes('-')) {
      // Both pre-release, compare pre-release parts
      const v1Pre = v1.split('-')[1];
      const v2Pre = v2.split('-')[1];
   ...
   * @param {string} projectDir - Project directory
   * @returns {string} Path to install manifest
   */
  getManifestPath(projectDir) {
    return path.join(projectDir, '.bmad-core', 'install-manifest.yaml');
  }
}

module.exports = { Installer, InstallModeDetector };
