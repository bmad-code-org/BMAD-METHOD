...
* **`bmad-create-epics-and-stories` deprecated to a forwarding shim** (#2672). Now a `v6-shims/` husk that forwards to `bmad-ticket`, which owns the ticket tree, the breakdown routes, and the coverage contract. Migrate `_bmad/custom/bmad-create-epics-and-stories.toml` overrides to `_bmad/custom/bmad-ticket.toml`. The `CE` trigger on the PM menu points at `bmad-ticket`, so "create epics and stories" keeps working. Net-new epics and stories no longer feed `sprint-status.yaml` — the tree's `board` and `frontier` verbs are the tracking for new work, and `sprint-status.yaml` stays in service for in-flight v6 stories.

### 🎁 Features

* **`bmad-ticket` — the ticket tree replaces the epics document** (#2672). One skill owns breakdown for the planning lane across three routes: refine a single ticket, slice scope into the detailed epic set, or incept one epic into its stories just-in-time when its work begins — so stories aren't written long before the reality they describe, and an epic envelope alone (description, goals, covers, linked inputs) supports T-shirt sizing without a single story written. Work lives as a recursive tree of epic folders and story files under `.bmad-obeya/` — the configurable work store, with the root `tickets/` bin as the default home — with `KEY-n` ids, explicit `depends_on` edges, and a coverage contract tying tickets back to the inventory they came from. New `ticket_tree.py` (`next-id`, `index`, `validate`, `list`, `frontier`, `board`, `coverage`, `graph`, `render`, `archive` — JSON-only output, 41 tests) owns scanning, id allocation, derived rollups, dependency-cycle detection, and the generated navigation map; new `update_ticket.py` (32 tests) owns guarded field writes with configurable status transitions, `depends_on` reference validation, and atomic writes. Leaves and nodes run separate lifecycles — `backlog → in-progress → review → done` for a story, `backlog → ready → in-progress → done` for an epic or project — and every move at both altitudes is a stored decision, never calculated from children, so an epic marked done cannot flip back when a story lands later. Progress against children is derived by `board` instead: information about an epic, not its state. Judgment stays with the LLM — what the epics are, where a slice boundary falls, when a ticket is ready. Both scripts are dual-homed: `src/scripts/` is canonical and the skill bundles a byte-identical copy, with `test_dual_home_sync.py` failing the build if the two drift. All three suites run under the new `npm run test:ticket`.
