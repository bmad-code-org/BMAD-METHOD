...
프로젝트에 BMad를 설정하려면 `npx bmad-method install`을 사용하세요. 하나의 명령으로 최초 설치, 업그레이드, 채널 전환, 스크립트 기반 CI 실행을 처리합니다. 이 페이지에서는 전체 사용법을 설명합니다.
...
선택한 모듈에 폐기 예정 호환 스킬이 남아 있다면 대화형 설치 과정에서 여섯 가지를 묻습니다.
...
6. 폐기 예정 호환 스킬을 설치할지 여부(새로 설치할 때의 기본값은 **No**)

기본값을 받아들이면 선택한 도구에 맞게 설정된 각 모듈의 최신 안정 릴리스가 설치됩니다.

프로젝트에 호환 스킬이 남아 있으면 Quick Update에서도 같은 질문을 합니다. 이때 기본값은 유지이므로 Enter만 눌러도 사용 중인 스킬이 갑자기 사라지지 않습니다. 제거하려면 **No**를 선택하세요.

이미 호환 스킬을 제거한 설치에서는 다시 묻지 않습니다. 호환 스킬을 유지하면 어떤 스킬이 남고 각각 어디로 연결되는지 설치 프로그램이 모두 보여줍니다. 이후 릴리스에서 호환 스킬이 완전히 사라지면 질문도 중단되며, 기존에 설치된 호환 스킬은 일반 업데이트 정리 과정에서 제거됩니다.
...
**설치 프로그램 바이너리가 core와 bmm 버전을 결정합니다.** 이 두 모듈은 별도 저장소에서 복제되지 않고 설치 프로그램 패키지에 함께 포함됩니다.
...
| `--modules <a,b,c>` | 정확한 모듈 집합입니다. core는 자동 추가됩니다. 기존 목록에 덧붙이는 방식이 아니므로 유지하려는 모듈을 모두 나열하세요 |
...
| `--shims` | 선택한 모듈이 아직 제공하는 폐기 예정 호환 스킬을 설치합니다 |
| `--no-shims` | 폐기 예정 호환 스킬을 설치하지 않습니다. 기존 설치를 업데이트할 때 사용하면 설치된 호환 스킬도 제거합니다 |
...
`--set <module>.<key>=<value>`는 모듈 설정 옵션을 비대화형으로 지정하며 여러 번 사용할 수 있습니다. 현재 모듈뿐 아니라 앞으로 추가되는 모듈에도 같은 방식으로 적용됩니다. 이 플래그는 설치가 끝난 뒤 값을 패치합니다. 설치 프로그램이 정상 흐름을 먼저 실행한 다음 `--set`은 각 값을 팀 범위의 `_bmad/config.toml` 또는 사용자 범위의 `_bmad/config.user.toml`에 업데이트하거나 추가합니다. `_bmad/<module>/config.yaml`에도 같은 값을 기록하므로 선언된 값은 다음 설치까지 이어집니다.
...
`--list-options`(인자 없음)는 설치 프로그램이 로컬에서 찾을 수 있는 모든 키를 나열합니다. 내장 모듈(`core`, `bmm`)과 현재 캐시된 공식 모듈이 포함됩니다. 캐시는 머신마다 다르고 지워질 수도 있습니다. 따라서 이전에 설치한 공식 모듈도 새 체크아웃이나 임시 CI 작업자에서는 다시 설치하기 전까지 목록에 나타나지 않습니다. 커뮤니티 및 커스텀 모듈은 여기서 열거하지 않습니다. 모듈의 `module.yaml`을 직접 읽어 선언된 키를 확인하세요.

**작동 방식:**

- **경로 선택.** 패치 단계는 먼저 `config.user.toml`에서 `[modules.<module>] <key>`(또는 `[core] <key>`)를 찾고, 있으면 그 파일을 업데이트합니다. 그렇지 않으면 팀 범위 `config.toml`에 씁니다. 그래서 `core.user_name`, `bmm.user_skill_level` 같은 사용자 범위 키는 `config.user.toml`에, 팀 범위 키는 `config.toml`에 들어갑니다. 설치 프로그램도 같은 기준으로 설정을 분리합니다.
...
- **선언된 키 유지.** `module.yaml`에 선언된 키 값은 `_bmad/<module>/config.yaml`에도 쓰이므로 다음 설치 때 프롬프트 기본값으로 유지됩니다.
- **미선언 키는 유지되지 않음.** 모듈 스키마가 선언하지 않은 키 값은 현재 설치의 `config.toml`에 들어가지만 다음 설치 때 다시 생성되지 않습니다. 매니페스트 작성기의 엄격한 스키마 분리 단계에서 알 수 없는 키를 제외하기 때문입니다. 계속 유지해야 한다면 `--set`을 다시 전달하거나 `_bmad/config.toml`을 직접 수정하세요.
...
레거시 core 단축 플래그(`--user-name`, `--output-folder` 등)는 하위 호환성을 위해 계속 동작하고 문서화되어 있지만, `--set core.user_name=...`과 동등합니다.
...
`sha` 필드는 git 기반 모듈(외부, 커뮤니티, URL 기반 커스텀)에 기록됩니다. 번들 모듈(core, bmm)과 로컬 경로 커스텀 모듈에는 없습니다. 이 모듈들의 코드는 복제 가능한 참조가 아니라 설치 프로그램 바이너리나 로컬 파일시스템에서 가져오기 때문입니다.

머신 간 재현성을 위해 같은 `--modules` 명령을 다시 실행하는 것에 의존하지 마세요. 안정 채널 설치는 **설치 시점**의 가장 높은 릴리스 태그로 해석되므로 나중에 다시 실행하면 그 사이 릴리스된 버전으로 설치됩니다. 대상 머신에서는 `manifest.yaml`에 기록된 태그를 명시적인 `--pin` 플래그로 전달하세요. 예:
