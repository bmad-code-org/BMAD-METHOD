...
이 스킬은 스크립트와 여러 서브에이전트로 코드베이스와 문서를 먼저 스캔합니다. 그런 다음 코드만으로는 알 수 없는 위험 요소, 변경 금지 영역, 조직 요구사항만 짧게 나누어 묻습니다. 생성 문서를 늘리지 않습니다. 대신 항상 로드되는 작은 `kernel.md`와 `project_knowledge` 폴더의 검증된 항목 번들을 만듭니다. 기존 `docs/` 폴더가 비대하더라도 내용을 더 보태지 않고 코드와 대조해 검증할 소스로 사용합니다.
...
`bmad-project-context`는 이 가운데 에이전트가 읽는 부분을 감사하고 유지합니다. 컨텍스트가 오래됐다고 느껴지면 감사를 실행하세요. 내용을 계속 쌓는 대신 줄이고 다시 검증합니다. 이전 `bmad-document-project` 워크플로는 더 이상 사용하지 않으며 이 스킬로 전달됩니다.
