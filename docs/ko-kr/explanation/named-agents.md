...
구체적 예: 팀이 Amelia에게 라이브러리 문서는 항상 Context7 MCP 도구를 사용하고 로컬 에픽 목록에 스토리가 없으면 Linear를 대체 경로로 사용하라고 하는 단일 파일을 커밋합니다. Amelia가 실행하는 모든 개발 워크플로(build, code-review, qa-generate)가 소스 편집이나 워크플로별 중복 없이 이 동작을 상속합니다.
