...
| **접근 방식** | 테스트를 빠르게 생성하고 나중에 반복해 개선 | 먼저 계획하고 요구사항과 연결해 추적 가능하게 생성 |
...
내장 QA 워크플로(`bmad-qa-generate-e2e-tests`)는 BMM(애자일 제품군) 모듈의 일부이며 개발자 에이전트에서 실행할 수 있습니다. 설정이나 추가 설치 없이 프로젝트의 기존 테스트 프레임워크를 사용해 실행 가능한 테스트를 빠르게 생성합니다.

**트리거:** 개발자 에이전트의 `QA` 또는 `bmad-qa-generate-e2e-tests`
...
워크플로는 테스트 요약을 프로젝트의 구현 산출물 폴더에 저장합니다.
...
TEA는 전문가 에이전트(Murat)와 엔터프라이즈급 테스트를 위한 9개 구조화된 워크플로를 제공하는 독립 모듈입니다. 테스트 생성을 넘어 테스트 전략, 위험 기반 계획, 품질 게이트, 요구사항 추적성을 다룹니다.
...
- 여러 기능의 테스트 우선순위를 위험도에 따라 정해야 하는 팀
- 릴리스 전 공식 품질 게이트가 있는 엔터프라이즈 환경
- 테스트를 작성하기 전에 전략부터 세워야 하는 복잡한 도메인
- 내장 QA 워크플로 하나만으로는 부족해진 프로젝트

## 테스트가 워크플로와 맞물리는 방식

QA 자동화 워크플로는 BMad Method 워크플로 맵의 단계 4(구현)에 나타납니다. **전체 에픽이 완료된 후**, 즉 에픽의 모든 스토리를 구현하고 코드 리뷰까지 마친 뒤 실행하도록 설계되었습니다. 일반적인 순서는 다음과 같습니다.

1. 에픽의 각 스토리를 Build(`BD` / `bmad-build`)로 구현하고 필요하면 코드 리뷰(`CR`)를 추가합니다
2. 에픽 완료 후 개발자 에이전트의 `QA` 또는 TEA의 자동화 워크플로로 테스트를 생성합니다
