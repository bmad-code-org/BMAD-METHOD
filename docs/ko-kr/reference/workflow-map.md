...
| 워크플로 | 목적 | 산출물 |
| --- | --- | --- |
| `bmad-architecture` | 기술적 결정을 명시적으로 만듭니다 | 기본 핵심 문서는 `ARCHITECTURE-SPINE.md`이며, 필요한 출력이나 프레젠테이션 형태로 확장해 씁니다 |
...
스토리 하나씩 구현합니다. 4단계 에픽 및 스토리 자동화도 이제 지원합니다. 사용자는 흐름에 얼마나 개입할지 고르면 됩니다. 전체 흐름을 타도 되고, 빠른 흐름으로 바로 넘어가도 됩니다.
