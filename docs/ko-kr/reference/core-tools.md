...
모든 BMad 설치에는 **핵심 모듈**이 포함됩니다. 프로젝트, 모듈, 단계에 관계없이 두루 쓰는 작은 스킬 모음입니다. 이 페이지에서는 네 가지 커널 도구와 세 가지 **사고 스킬**(브레인스토밍, 아이디어 단련, 파티 모드), 모두 일곱 가지 핵심 스킬을 설명합니다.
...
기존 `bmad-editorial-review`, `bmad-editorial-review-prose`, `bmad-editorial-review-structure`, `bmad-review-adversarial-general`, `bmad-review-edge-case-hunter`, `bmad-review-verification-gap` 스킬은 모두 `bmad-review`로 통합되었습니다. 별도의 편집 스킬은 문서 편집 렌즈가 대신합니다. 여기에 나열한 기존 ID 6개도 호환성 포워더로 계속 작동합니다.
:::
...
| 렌즈 | 적용 대상 | 기법 |
| --- | --- | --- |
| **적대적** (`adversarial`) | 모든 내용 | 문제가 있다고 가정해 틀린 것뿐 아니라 빠진 것도 찾습니다 |
