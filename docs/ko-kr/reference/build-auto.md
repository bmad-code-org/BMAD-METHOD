---
title: 자율 개발 루프
description: bmad-build-auto를 1회 반복 실행 단위로 사용해 Build 구현 모델을 자동화하는 방법
sidebar:
  order: 7
---

`bmad-build-auto`는 표준 [Build](../explanation/build.md) 구현 모델을 사람 개입 없이 자동화하는 진입점입니다. 직접 입력한 의도부터 계획된 작업까지 Build와 같은 범위를 받습니다. 명확화·계획·구현·리뷰 단계는 그대로 유지하면서 오케스트레이터가 후속 처리할 수 있는 종료 상태를 제공합니다. 구현 루프를 자동화할 뿐 별도의 구현 경로를 정의하지는 않습니다.

중요한 아키텍처 경계는 다음과 같습니다. `bmad-build-auto`는 구현 실행과 그 과정에서 만든 사양 산출물을 담당하지만 백로그 정책까지 다루지는 않습니다. 리뷰에서 실제 문제를 찾았지만 현재 스토리와 무관하다면, 스킬은 자신이 관리하는 사양에 그 발견 사항을 기록하고 멈춥니다. 대기열에 넣을지, 중복을 제거할지, 상위로 전달할지, 무시할지는 오케스트레이터가 결정합니다.
