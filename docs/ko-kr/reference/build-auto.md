---
title: 자율 개발 루프
description: bmad-build-auto를 단일 반복 작업자로 사용해 Build 구현 모델을 자동화하는 방법
sidebar:
  order: 7
---

`bmad-build-auto`는 표준 [Build](../explanation/build.md) 구현 모델을 사람 개입 없이 자동화하는 진입점입니다. 직접 입력한 의도부터 계획된 작업까지 Build와 같은 범위를 받습니다. 명확화·계획·구현·리뷰 단계는 그대로 유지하면서 오케스트레이터가 처리할 최종 상태를 제공합니다. 구현 루프를 자동화할 뿐 별도의 구현 경로를 정의하지는 않습니다.

중요한 아키텍처 경계는 다음과 같습니다. `bmad-build-auto`는 구현 실행과 그 과정에서 만든 사양 산출물을 소유하지만 백로그 정책까지 소유하지는 않습니다. 리뷰에서 실제 문제이지만 현재 스토리의 문제가 아닌 항목을 찾으면, 스킬은 자신이 소유한 사양에 그 발견 사항을 기록하고 멈춥니다. 대기열에 넣을지, 중복을 제거할지, 상위로 전달할지, 무시할지는 오케스트레이터가 결정합니다.

## 하는 일

`bmad-build-auto`는 사람 개입 없는 개발 루프를 한 번 실행합니다.
...
실행 환경에서 하위 에이전트를 사용할 수 있어야 합니다. 그렇지 않으면 워크플로는 `no subagents` 조건과 `blocked` 상태로 멈춥니다. 예를 들어 "Claude, 2-10번 스토리를 구현하되, 각 스토리마다 `bmad-build-auto` 스킬을 실행하는 하위 에이전트를 사용해 줘"처럼 이 스킬을 하위 에이전트 세션에서 호출한다면 그 세션도 자체 하위 에이전트를 생성할 수 있어야 합니다.
...
- `summary` — 미룬 문제를 한 문장으로 설명합니다
- `evidence` — 해당 발견 사항이 실제 문제인 근거입니다
- `location` — 선택 사항인 file:line 또는 컴포넌트 힌트입니다
- `severity` — 선택 사항인 최종 분류 심각도(`high`, `medium`, `low`)입니다

이 목록은 의도적으로 백로그 역할을 하지 않습니다. 기계가 읽을 수 있는 리뷰 결과일 뿐입니다. 티켓을 만들지, 중앙 대기열에 추가할지, 실행 간 중복을 연결할지, 아무것도 하지 않을지는 오케스트레이터가 결정해야 합니다.
...
- `baseline_revision` — 구현 전의 전체 표준 리비전입니다. 버전 관리가 없으면 `NO_VCS`입니다.
...
`intent gap`은 실행 중 생긴 질문에 기록된 의도만으로 답할 수 없다는 뜻입니다. 코드가 만들어지기 전인 계획 단계에서 멈출 수도 있고 리뷰 단계에서 멈출 수도 있습니다. 리뷰 중 이 조건으로 멈추면 작업 트리를 평소처럼 되돌립니다. 다만 먼저 시도한 변경은 `{implementation_artifacts}` 아래의 패치 파일로 저장하고 사양의 분류 기록과 중단 출력에 경로를 기록합니다. 이 패치는 워크플로가 의도를 어떻게 해석해 구현했는지 보여 주는 구체적인 근거입니다. 그 해석이 맞다면 `git apply`로 패치를 적용하고 사양 상태를 `in-review`로 바꾸세요. 처음부터 다시 실행하지 않고 해당 변경의 리뷰를 이어갈 수 있습니다.

## 출력 산출물

워크플로는 나중에 다시 확인할 수 있도록 실행 결과를 영구 산출물로 남깁니다.
...
- 사양 프런트매터의 `deferred:` 목록에서 미룬 발견 사항을 읽습니다
- 스토리가 여러 개라면 `baseline_revision..<next story's baseline_revision>`, 다음 스토리가 아직 없다면 종료 시점의 `baseline_revision..HEAD`로 해당 스토리의 커밋을 식별합니다
