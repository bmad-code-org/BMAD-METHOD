...
| "Review the stories"                     | Reviews and improves the entries with you: descriptions, checks, order, and prerequisites.  |
...
An entry has no file until you pull it. Pulling writes the story file. `bmad-build` plans the story's acceptance criteria when it builds, from the epic and the entry, so detail is not written months before it is used.
