...
An entry has no file until you pull it. Pulling writes the story file, marked `refined: false`. `bmad-build` plans the story's acceptance criteria when it builds, from the epic and the entry, so detail is not written months before it is used.

A story can be blocked by a story in another epic, or by a whole epic. Ask "what's next?" about the initiative to see every epic at once.
