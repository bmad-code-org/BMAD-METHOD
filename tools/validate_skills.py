...
Validates 13 deterministic rules across skill directories.
...
STEP_FILENAME_REGEX_DISPLAY = r"/^step-\d{2}[a-z]?-[a-z0-9-]+\.md$/"
...
def validate_skill(skill_dir: str) -> list[dict]:
    ...
    if os.path.exists(steps_dir) and os.path.isdir(steps_dir):
        ...
        for step_file in step_files:
            if not STEP_FILENAME_REGEX.search(step_file):
                findings.append(
                    _finding(
                        "STEP-01",
                        "Step File Naming",
                        "MEDIUM",
                        os.path.join(step_dir_name, step_file),
                        f'Filename "{step_file}" does not match pattern: {STEP_FILENAME_REGEX_DISPLAY}',
                        "Rename to step-NN-description.md (NN = zero-padded number, optional letter suffix).",
                    )
                )

        for step_file in step_files:
            step_path = os.path.join(steps_dir, step_file)
            step_content = safe_read_file(step_path, findings, os.path.join(step_dir_name, step_file))
            ...
            if step_fm:
                if "name" in step_fm:
                    findings.append(
                        _finding(
                            "STEP-06",
                            "Step File Frontmatter: No name or description",
                            "MEDIUM",
                            os.path.join(step_dir_name, step_file),
                            "Step file frontmatter contains `name:` — this is metadata noise.",
                            "Remove `name:` from step file frontmatter.",
                        )
                    )
                if "description" in step_fm:
                    findings.append(
                        _finding(
                            "STEP-06",
                            "Step File Frontmatter: No name or description",
                            "MEDIUM",
                            os.path.join(step_dir_name, step_file),
                            "Step file frontmatter contains `description:` — this is metadata noise.",
                            "Remove `description:` from step file frontmatter.",
                        )
                    )

        step_count = sum(1 for f in step_files if f.startswith("step-"))
        if step_count > 0 and (step_count < 2 or step_count > 10):
            if step_count < 2:
                detail = f"Only {step_count} step file found — consider inlining into workflow.md."
            else:
                detail = f"{step_count} step files found — more than 10 risks LLM context degradation."
            findings.append(
                _finding(
                    "STEP-07",
                    "Step Count",
                    ...
                    step_dir_name + "/",
                    detail,
                    "Consider consolidating steps." if step_count > 10 else "Consider expanding or inlining.",
                )
            )
