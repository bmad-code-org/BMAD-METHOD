...
class TestRules(ProjectCase):
    ...
    def test_step_01_filename_uses_js_regex_literal(self):
        skill = self.valid(
            "bmad-step-name",
            {
                "steps/readme.md": "# not a step\n",
                "steps/step-01-init.md": "# ok\n",
            },
        )
        findings = findings_by_rule(self.findings_for(skill), "STEP-01")
        self.assertEqual(len(findings), 1)
        self.assertEqual(findings[0]["file"], "steps/readme.md")
        self.assertIn("/^step-\\d{2}[a-z]?-[a-z0-9-]+\\.md$/", findings[0]["detail"])

    def test_step_06_emits_two_findings_when_both_keys_present(self):
        skill = self.valid(
            "bmad-step-fm",
            {
                "steps/step-01-init.md": "---\nname: init\ndescription: start\n---\n\n# Goal\n",
                "steps/step-02-next.md": "# Goal\n",
            },
        )
        findings = findings_by_rule(self.findings_for(skill), "STEP-06")
        self.assertEqual(len(findings), 2)
        details = {f["detail"] for f in findings}
        self.assertIn("Step file frontmatter contains `name:` — this is metadata noise.", details)
        self.assertIn("Step file frontmatter contains `description:` — this is metadata noise.", details)

    def test_step_07_count_low_and_high(self):
        one = self.valid("bmad-one-step", {"steps/step-01-only.md": "# Goal\n"})
        one_f = findings_by_rule(self.findings_for(one), "STEP-07")
        self.assertEqual(len(one_f), 1)
        self.assertEqual(one_f[0]["file"], "steps/")
        self.assertEqual(one_f[0]["severity"], "LOW")
        self.assertIn("Only 1 step file found", one_f[0]["detail"])

        files = {f"steps/step-{i:02d}-item.md": "# Goal\n" for i in range(1, 12)}
        many = self.valid("bmad-many-steps", files)
        many_f = findings_by_rule(self.findings_for(many), "STEP-07")
        self.assertEqual(len(many_f), 1)
        self.assertIn("11 step files found", many_f[0]["detail"])

        two = self.valid(
            "bmad-two-steps",
            {"steps/step-01-a.md": "# a\n", "steps/step-02-b.md": "# b\n"},
        )
        self.assertEqual(findings_by_rule(self.findings_for(two), "STEP-07"), [])
