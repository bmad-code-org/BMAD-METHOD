...
3. Resolve central config by merging the four project config layers in order: the two base config files in `{project-root}/_bmad/`, then their two matching override files in `{project-root}/_bmad/custom/`; from the merged `core` table resolve `{user_name}`, `{communication_language}`, `{document_output_language}`, `{output_folder}`, `{project_name}`; `{date}` is today. On failure or missing values, use neutral defaults; never block.
