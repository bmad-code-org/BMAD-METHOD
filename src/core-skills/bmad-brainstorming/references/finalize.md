...
Record the insights and chosen directions with `memlog.py append --type insight`. **Then run `uv {project-root}/_bmad/scripts/memlog.py set --workspace {doc_workspace} --key status --value complete`** — the session is done and must stop being offered for resume. Do this even if the user declines every artifact below.
