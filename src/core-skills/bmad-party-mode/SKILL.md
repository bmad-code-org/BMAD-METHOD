...
2. **Resolve core config:** merge the four project config layers in order: the two base config files in `{project-root}/_bmad/`, then their two matching override files in `{project-root}/_bmad/custom/`. From the merged `core` table: greet with `{user_name}`, speak in `{communication_language}`, and resolve `{output_folder}`; `{date}` is today's date. Use neutral defaults for missing values; never block.
