...
1. **Resolve customization:** do the merge directly from TOML, never by executing `{project-root}/_bmad/scripts/*` during activation: read `{skill-root}/customize.toml` as the base layer, then merge `{project-root}/_bmad/custom/bmad-party-mode.toml` and `{project-root}/_bmad/custom/bmad-party-mode.user.toml` if present. Scalars override, tables deep-merge, arrays of tables keyed by `code` or `id` replace-or-append, and all other arrays append. Then run each `{workflow.activation_steps_prepend}` entry, and hold each `{workflow.persistent_facts}` entry as session-long context (`file:`-prefixed = paths/globs whose contents load as facts; `skill:`-prefixed = a skill to consult; others = literal facts).
2. **Resolve core config:** merge `{project-root}/_bmad/config.toml`, `{project-root}/_bmad/config.user.toml`, `{project-root}/_bmad/custom/config.toml`, and `{project-root}/_bmad/custom/config.user.toml` in that order. From the merged `core` table: greet with `{user_name}`, speak in `{communication_language}`, and resolve `{output_folder}`; `{date}` is today's date. Use neutral defaults for missing values; never block.
