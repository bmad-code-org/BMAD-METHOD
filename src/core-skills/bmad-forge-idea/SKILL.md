...
3. Resolve central config by merging the four project config layers in order: the two base config files in `{project-root}/_bmad/`, then their two matching override files in `{project-root}/_bmad/custom/`; from the merged `core` table read `{user_name}`, `{communication_language}`, `{output_folder}`. On failure use neutral defaults; never block. Greet `{user_name}` in `{communication_language}` and stay in it.
