...
1. Resolve customization directly from TOML, never by executing `{project-root}/_bmad/scripts/*` during activation: read `{skill-root}/customize.toml` as the base layer, then merge `{project-root}/_bmad/custom/bmad-forge-idea.toml` and `{project-root}/_bmad/custom/bmad-forge-idea.user.toml` if present. Scalars override, tables deep-merge, arrays of tables keyed by `code` or `id` replace-or-append, and all other arrays append. Apply the resolved `{workflow.*}` values throughout.
...
3. Resolve central config by merging `{project-root}/_bmad/config.toml`, `{project-root}/_bmad/config.user.toml`, `{project-root}/_bmad/custom/config.toml`, and `{project-root}/_bmad/custom/config.user.toml` in that order; from the merged `core` table read `{user_name}`, `{communication_language}`, `{output_folder}`. On failure use neutral defaults; never block. Greet `{user_name}` in `{communication_language}` and stay in it.
