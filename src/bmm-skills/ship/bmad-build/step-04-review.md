...
1. Once every layer has reported — and not before — render a verdict on each finding on its own, ahead of any deduplication or grouping. For each finding:
   ...
   - Every finding gets one row in the `## Review Triage Log` section of `{spec_file}` — verdict plus its evidence in a sentence or two; never drop, merge, or silently skip one. `false` findings take no further action. `maybe-false` findings route to defer. `critical`, `medium`, and `minor` findings continue to grouping.
