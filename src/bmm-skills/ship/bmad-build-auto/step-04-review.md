...
2. Assign severity to each finding by consequence for the artifact's main consumer (software user, document reader, etc).
   ...
   - `medium`: tolerable
   - `high`: intolerable
3. Route each finding into exactly one triage category. The first three categories are **this story's problem** — caused or exposed by the current change and demonstrated. The last two are **not this story's problem**.
   Reviewers report claims, not demonstrations — by design they review the change without full-codebase context. Verify substance yourself before routing: a finding counts as demonstrated only when you can produce its trigger — trace a reachable path, construct the input, or run the failing case. A loud failure on a state you could not demonstrate the system producing is correct handling, not a defect. Judge each finding by its demonstrated trigger, never by the remedy it proposes.
   ...
   - **patch** — caused by the change; shows a demonstrated defect, missing coverage for a specific case, or a broken gate or convention, and its smallest fix is trivial, adds no public surface, and guards no state you did not demonstrate. Just part of the diff. A demonstrated finding whose smallest fix fails any of those conditions is not a patch: route it to intent_gap when the spec does not settle that fix (e.g., new public surface the spec never authorized), otherwise to bad_spec.
   - **defer** — every other real finding: pre-existing issues, improvement ideas, and claims you believe but could not demonstrate. Collect for later focused attention.
   - **reject** — only noise. Drop silently. When unsure between defer and reject, prefer reject — only defer findings you are confident are real.
