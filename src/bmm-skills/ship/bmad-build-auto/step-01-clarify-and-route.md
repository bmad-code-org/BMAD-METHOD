...
followup_pass: '' # set at runtime to `true` when a `done` spec whose previous pass recommended a follow-up review is re-dispatched; empty otherwise
---
...
- `done` → set `review_loop_iteration` to `0` in the frontmatter; set `{followup_pass}` to `true` only if the spec's `followup_review_recommended` is `true`; then **EARLY EXIT** to `[[bmad-snapshot:step-04-review.md]]` for a fresh review pass. (A `done` spec is a completed run, so this starts a follow-up review, not a resumption.)
...
- **If exactly one matches**, set `spec_file` to that path.
  ...
  - Any other recognized `status`: **EARLY EXIT** using the same routing as above, including the `review_loop_iteration` reset and the `{followup_pass}` rule for `done`. One difference: a `blocked` story HALTs with blocking condition `story already blocked`, not `blocked spec supplied` — the caller did not supply this file; build-auto found it by id.
