...
6. Warning check. Add every warning step-01 carried forward — `multiple-goals`, and on the ticket path `hitl` and any unmet dependency — to `{spec_file}` frontmatter `warnings`. If `{spec_file}` exceeds 1600 tokens, add `oversized` to frontmatter `warnings`. Continue either way.
