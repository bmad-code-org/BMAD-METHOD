...
3. **Adopt an in-repo manifest (`[C]` route).** For a custom-URL clone, check for `bmad-template.md` at the staging root. If present, read it fully and adopt it as the template manifest for the rest of the run — rerun the preflight for any newly declared `requires`, and summarize its `## Environment` section to the user, since the proposal in step 2 could not. If absent, continue with the provisional entry; env wiring and verification will be discovered from the repo's README and manifests.
