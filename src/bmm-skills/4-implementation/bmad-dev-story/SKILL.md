...
<workflow>
  ...
  <step n="1" goal="Find next ready story and load it" tag="sprint-status">
    <check if="{{story_path}} is provided">
      ...
      <action>Set `detail_design_file` = `{implementation_artifacts}/detail-design/{{story_key}}-detail-design.md`</action>
      <goto anchor="detail_design_gate" />
    </check>
    ...
    <check if="{{sprint_status}} file exists">
      ...
      <action>Find the FIRST story (by reading in order from top to bottom) where:
        ...
        - Status value equals "in-progress" or "ready-for-dev"
      </action>

      <action>Prefer the first "in-progress" story. If none exist, use the first "ready-for-dev" story.</action>

      <check if="no ready-for-dev or in-progress story found">
        <output>📋 No ready-for-dev or in-progress stories found in sprint-status.yaml

          **Current Sprint Status:** {{sprint_status_summary}}

          **What would you like to do?**
          1. Run `create-story` to create next story from epics with comprehensive context
          2. Run `*validate-create-story` to improve existing stories before detail design and development (recommended quality check)
          3. Specify a particular story file to develop (provide full path)
          4. Check {{sprint_status}} file to see current sprint status

          💡 **Tip:** Stories in `ready-for-dev` should go through `bmad-detail-design` before `bmad-dev-story`. Consider running
          `validate-create-story` first if story quality is still uncertain.
        </output>
    ...
    <check if="{{sprint_status}} file does NOT exist">
      ...
      <action>Find stories with "ready-for-dev" or "in-progress" status in files</action>
      <action>Look for story files matching pattern: *-*-*.md</action>
      <action>Read each candidate story file to check Status section</action>

      <check if="no ready-for-dev or in-progress stories found in story files">
        <output>📋 No ready-for-dev or in-progress stories found

          **Available Options:**
          1. Run `create-story` to create next story from epics with comprehensive context
          2. Run `*validate-create-story` to improve existing stories before detail design
      ...
      <check if="ready-for-dev or in-progress story found in files">
    ...
    <anchor id="detail_design_gate" />

    <check if="{{detail_design_file}} does not exist">
      <output>📐 Story {{story_key}} is missing its required detail design artifact.

        **Required before development:**
        - Run `bmad-detail-design`
        - Target artifact: {{detail_design_file}}

        `bmad-dev-story` does not begin implementation until the detail design exists.
      </output>
      <action>HALT - Route to bmad-detail-design for {{story_key}}</action>
    </check>
  ...
  <step n="2" goal="Load project context and story information">
    ...
    <action>Load {{detail_design_file}} and treat it as required implementation context</action>
    <action>Parse sections: Story, Acceptance Criteria, Tasks/Subtasks, Dev Notes, Dev Agent Record, File List, Change Log, Status</action>
    <action>Load comprehensive context from story file's Dev Notes section</action>
    <action>Extract developer guidance from Dev Notes: architecture requirements, previous learnings, technical specifications</action>
    <action>Extract developer guidance from the detail design: contracts, data changes, flow/error handling, test expectations, and observability requirements</action>
    <action>Use enhanced story context to inform implementation decisions and approaches</action>
