...
- **God-class / size trajectory** — files that grew past a healthy size *over the epic*, invisible per-commit because each session added only a little. The `git_evidence.py` pre-pass (Phase 1) reports added / deleted / net per file across the range; read its `files` output and flag the large or fast-accelerating ones. Whether a flagged file is genuinely a god-class or legitimately large stays your judgment.
