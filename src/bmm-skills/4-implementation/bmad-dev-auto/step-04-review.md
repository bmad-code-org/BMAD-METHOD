...
2. Assign severity to each finding by consequence for the artifact's main consumer (software user, document reader, etc).
   ...
   Proof of impact is required only above `low`. A cost or behavior change that is real in the code, but whose impact on the user was not shown, rates `low`. "The impact was not shown" is a reason to rate a finding `low`, never a reason to reject it.
3. Missing verification is a finding. If the change alters behavior, and no test or check would fail when that behavior breaks somewhere a user or caller would notice, that is a finding: rate it by what happens if the behavior breaks silently, and route it like any other finding (usually `patch` — the fix is the missing test). Do not put it in a cannot-verify list: cannot-verify is only for claims you could not check, not for gaps you have confirmed.
4. Route each finding into exactly one triage category. The first three categories are **this story's problem** — caused or exposed by the current change. The last two are **not this story's problem**.
   ...
   - **reject** — noise, false positive, or handled elsewhere; drop silently. Rejecting a finding on the current change is allowed only when what you checked covers EVERY case the finding claims: say exactly what you checked and why that closes all of them. If your check closes only some of the cases, the finding stays for the remaining cases, at whatever severity those deserve. Wrongly rejecting a real finding is worse than keeping a `low` one. When unsure between defer and reject on a pre-existing issue, prefer reject — only defer findings you are confident are real.
5. Append a new entry to the `## Review Triage Log` section in `{spec_file}`, in this format:
   ```markdown
   ### {date} — Review pass
   - intent_gap: count
   - bad_spec: count
