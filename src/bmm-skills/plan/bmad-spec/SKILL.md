...
On acceptance, invoke `bmad-ticket` with the spec folder as input — `SPEC.md` plus its `companions:` are the source documents — and name the route: **slice** to produce the epic set (with `covers:` carrying this spec's `CAP-N` ids verbatim), **incept** for one named epic's stories. "Break this into stories" on open scope means slice first, then offer incept per epic — say so, so epics-only output isn't a surprise. That trace spine is why capability IDs must stay stable (Spec Law rule 6): ticket-side coverage validates against them.
