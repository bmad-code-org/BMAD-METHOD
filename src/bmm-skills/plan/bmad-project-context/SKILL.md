...
- **Orchestrate the scanning.** Mining a repo, docs folder, or org input fans out to parallel subagents — one per source or area, each returning ONLY a claim list (claim, evidence path, inferred|needs-confirmation). You interrogate and decide; you never grind a large tree through your own context. Note source paths for subagent scanning; don't read them yourself first. If subagents are unavailable, scan sequentially, smallest sources first.
