...
async function runTests() {
  ...
  // ============================================================
  // Test Suite 49: standalone modules — discovery, hidden filtering, deps
  // ============================================================
  console.log(`${colors.yellow}Test Suite 49: standalone module discovery, hidden filtering, dependency union${colors.reset}\n`);

  try {
    const { OfficialModules } = require('../tools/installer/modules/official-modules');
    const om49 = new OfficialModules();
    const { modules: available49 } = await om49.listAvailable();
    const byId49 = new Map(available49.map((m) => [m.id, m]));

    // Discovery: the bundle and its atoms register as built-in local modules.
    assert(byId49.has('bmad-analysis'), 'listAvailable discovers the bmad-analysis bundle module');
    assert(byId49.has('bmad-brainstorming'), 'listAvailable discovers the bmad-brainstorming standalone module');
    assert(byId49.has('bmad-party-mode'), 'listAvailable discovers the bmad-party-mode standalone module');
    assert(byId49.has('bmad-forge-idea'), 'listAvailable discovers the bmad-forge-idea standalone module');

    // findModuleSource resolves a standalone module to its src/standalone-skills dir.
    const brainSrc49 = await om49.findModuleSource('bmad-brainstorming', { silent: true });
    assert(
      typeof brainSrc49 === 'string' && brainSrc49.replaceAll('\\', '/').endsWith('src/standalone-skills/bmad-brainstorming'),
      'findModuleSource resolves a standalone module to its source directory',
    );

    // Hidden metadata parsed from module.yaml.
    assert(byId49.get('bmad-brainstorming').hidden === true, 'standalone atom parses hidden: true');
    assert(byId49.get('bmad-party-mode').hidden === true, 'bmad-party-mode parses hidden: true');
    assert(byId49.get('bmad-forge-idea').hidden === true, 'bmad-forge-idea parses hidden: true');
    assert(byId49.get('bmad-analysis').hidden === false, 'visible bundle module is not hidden');
    assert(byId49.get('bmm').hidden === false, 'built-in bmm module is not hidden');

    // Picker visibility rule (mirrors UI._selectOfficialModules): hidden modules
    // drop out unless already installed.
    const pickerVisible = (installed) => available49.filter((m) => !(m.hidden && !installed.has(m.id))).map((m) => m.id);
    const visibleFresh49 = pickerVisible(new Set());
    assert(!visibleFresh49.includes('bmad-brainstorming'), 'hidden atom is filtered out of a fresh picker');
    assert(visibleFresh49.includes('bmad-analysis'), 'visible bundle survives the picker filter');
    assert(visibleFresh49.includes('bmm'), 'non-hidden built-in survives the picker filter');
    const visibleWithAtom49 = pickerVisible(new Set(['bmad-brainstorming']));
    assert(visibleWithAtom49.includes('bmad-brainstorming'), 'an already-installed hidden atom stays visible in the picker');

    // Dependency union: selecting the bundle pulls in its atoms; core stays first.
    const expanded49 = await om49.resolveModuleDependencies(['core', 'bmad-analysis']);
    assert(expanded49[0] === 'core', 'resolveModuleDependencies keeps core first');
    for (const atom of ['bmad-brainstorming', 'bmad-party-mode', 'bmad-forge-idea']) {
      assert(expanded49.includes(atom), `dependency union pulls in ${atom}`);
    }

    // Dedupe: bmm + bundle + an explicitly-selected atom yields no duplicates.
    const expanded49b = await om49.resolveModuleDependencies(['core', 'bmm', 'bmad-analysis', 'bmad-brainstorming']);
    const counts49 = new Map();
    for (const c of expanded49b) counts49.set(c, (counts49.get(c) || 0) + 1);
    assert(
      [...counts49.values()].every((n) => n === 1),
      'dependency union dedupes across bmm + bundle + explicit atom',
    );
    assert(
      expanded49b.filter((c) => c === 'bmad-brainstorming').length === 1,
      'an explicitly-selected atom is not duplicated by the bundle',
    );

    // Modules with no dependencies pass through unchanged (atoms declare none).
    const expanded49c = await om49.resolveModuleDependencies(['core', 'bmad-party-mode']);
    assert(
      JSON.stringify(expanded49c) === JSON.stringify(['core', 'bmad-party-mode']),
      'modules with no dependencies pass through unchanged',
    );

    // Unknown dependency codes warn-and-skip; resolution never throws.
    const om49d = new OfficialModules();
    om49d.listAvailable = async () => ({
      modules: [
        { id: 'fake-bundle', hidden: false, defaultSelected: false, dependencies: ['bmad-brainstorming', 'does-not-exist'] },
        { id: 'bmad-brainstorming', hidden: true, defaultSelected: false, dependencies: [] },
      ],
    });
    om49d.externalModuleManager = { listAvailable: async () => [] };
    let threw49;
    let unknownResult49;
    try {
      unknownResult49 = await om49d.resolveModuleDependencies(['fake-bundle']);
    } catch (error) {
      threw49 = error;
    }
    assert(!threw49, 'resolveModuleDependencies never throws on an unknown dependency', threw49?.stack);
    assert(unknownResult49.includes('bmad-brainstorming'), 'known dependencies still resolve when a sibling is unknown');
    assert(!unknownResult49.includes('does-not-exist'), 'unknown dependency code is skipped, not added');
  } catch (error) {
  ...
  // ============================================================
  // Test Suite 50: skill-less bundle module installs cleanly
  // ============================================================
  console.log(`${colors.yellow}Test Suite 50: skill-less bundle module install${colors.reset}\n`);
  ...
  try {
    ...
    const root50 = await fs.mkdtemp(path.join(os.tmpdir(), 'bmad-bundle-'));
    const bmadDir50 = path.join(root50, '_bmad');
    await fs.ensureDir(path.join(bmadDir50, '_config'));

    const om50 = new OfficialModules();
    let installErr50;
    let res50;
    try {
      res50 = await om50.install('bmad-analysis', bmadDir50, () => {}, { skipModuleInstaller: true, silent: true });
    } catch (error) {
      installErr50 = error;
    }
    assert(!installErr50, 'skill-less bundle installs without throwing', installErr50?.stack);
    assert(res50 && res50.success === true, 'install() reports success for the skill-less bundle');

    const modDir50 = path.join(bmadDir50, 'bmad-analysis');
    assert(await fs.pathExists(path.join(modDir50, 'module-help.csv')), 'bundle module-help.csv is copied into the install');
    assert(await fs.pathExists(path.join(modDir50, 'README.md')), 'bundle README.md is copied into the install');
    assert(!(await fs.pathExists(path.join(modDir50, 'module.yaml'))), 'module.yaml is filtered out of the install (install-time only)');

    // Manifest skill collection tolerates a module with zero skills.
    const mg50 = new ManifestGenerator();
    mg50.bmadDir = bmadDir50;
    mg50.bmadFolderName = '_bmad';
    mg50.updatedModules = ['bmad-analysis'];
    let collectErr50;
    try {
      await mg50.collectSkills();
    } catch (error) {
      collectErr50 = error;
    }
    assert(!collectErr50, 'collectSkills does not crash on a zero-skill module', collectErr50?.stack);
    assert(mg50.skills.length === 0, 'a skill-less bundle contributes no skills to the manifest');

    // Help-catalog merge tolerates the skill-less bundle.
    const inst50 = new Installer();
    let mergeErr50;
    try {
      await inst50.mergeModuleHelpCatalogs(bmadDir50, []);
    } catch (error) {
      mergeErr50 = error;
    }
    assert(!mergeErr50, 'mergeModuleHelpCatalogs does not crash on a skill-less bundle', mergeErr50?.stack);
    assert(
      await fs.pathExists(path.join(bmadDir50, '_config', 'bmad-help.csv')),
      'merged help catalog is written with a skill-less bundle present',
    );

    await fs.remove(root50);
  } catch (error) {
  ...
  // ============================================================
  // Test Suite 51: quickUpdate expands declared module dependencies
  // ============================================================
  console.log(`${colors.yellow}Test Suite 51: quickUpdate expands declared module dependencies${colors.reset}\n`);

  {
    // Scenario: an existing core+bmm install runs Quick Update after bmm
    // gained `dependencies` (the standalone analysis atoms). quickUpdate must
    // expand the installed-module list through resolveModuleDependencies —
    // otherwise the atoms are never installed and IDE cleanup deletes the
    // skills that moved out of core.
    const { Installer } = require('../tools/installer/core/installer');
    const { OfficialModules } = require('../tools/installer/modules/official-modules');

    const root51 = await fs.mkdtemp(path.join(os.tmpdir(), 'bmad-quick-update-'));
    const bmadDir51 = path.join(root51, '_bmad');
    await fs.ensureDir(path.join(bmadDir51, '_config'));
    await fs.ensureDir(path.join(bmadDir51, 'core'));
    await fs.writeFile(
      path.join(bmadDir51, '_config', 'manifest.yaml'),
      ['installation:', '  version: 6.10.0', 'modules:', '  - core', '  - bmm', '  - ghost-module', 'ides:', '  - claude-code', ''].join(
        '\n',
      ),
    );

    // Neutralize the interactive/config-collection surface; keep dependency
    // resolution and module filtering real.
    const origLoadExistingConfig = OfficialModules.prototype.loadExistingConfig;
    const origCollectQuick = OfficialModules.prototype.collectModuleConfigQuick;
    OfficialModules.prototype.loadExistingConfig = async function () {};
    OfficialModules.prototype.collectModuleConfigQuick = async function () {
      return false;
    };
    ...
    try {
      ...
      inst51.externalModuleManager = {
        resolveCanonicalCode: async (id) => id,
        listAvailable: async () => [],
      };
      let captured51 = null;
      inst51.install = async (cfg) => {
        captured51 = cfg;
        return { success: true };
      };

      const result51 = await inst51.quickUpdate({ directory: root51 });

      assert(captured51 !== null, 'quickUpdate delegates to install()');
      for (const atom of ['bmad-brainstorming', 'bmad-party-mode', 'bmad-forge-idea']) {
        assert(captured51.modules.includes(atom), `quickUpdate expands bmm dependencies to include ${atom}`);
      }
      assert(captured51.modules[0] === 'core', 'quickUpdate keeps core first after dependency expansion');
      assert(captured51.modules.filter((m) => m === 'bmm').length === 1, 'dependency expansion does not duplicate installed modules');
      assert(
        result51.skippedModules.includes('ghost-module'),
        'modules without local source are still preserved (skipped), not dropped by expansion',
      );
      assert(!captured51.modules.includes('ghost-module'), 'unavailable modules stay out of the update set');
    } catch (error) {
