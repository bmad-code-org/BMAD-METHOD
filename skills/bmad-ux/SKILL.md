...
3. Resolve config: `uv run {project-root}/_bmad/scripts/resolve_config.py --project-root {project-root} --key core.project_name --key core.output_folder --key modules.bmm.active_initiative`. `{date}` is the current system datetime. `{slug}` is what the design is about, in kebab-case: the run lands in `ux-{slug}/ux-{slug}.md`.
