...
2. **Resolve core config:** `uv run --no-project {project-root}/_bmad/scripts/resolve_config.py --project-root {project-root}`. From the merged JSON's `core` table resolve `{output_folder}`; `{date}` is today's date. Greet the user.
