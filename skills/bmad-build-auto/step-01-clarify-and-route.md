...
A file whose frontmatter `type` is `story`, `spike`, or `bug` is a ticket, not a spec, whatever its `status`: ingest it as the intent, never set `spec_file` to it, and never write to it.
