...
4. **Triage open items.** All Open Questions, `[ASSUMPTION]` tags, `[NOTE FOR PM]` callouts, `[UNLOGGED: ...]` tags. Phase-blockers (would make the PRD unsafe for UX/architecture/epics) surfaced one at a time and resolved; non-blockers deferred with owner + revisit condition logged via `memlog.py append`. `[UNLOGGED: ...]` tags are reconciled here: replay each into `.memlog.md` via `memlog.py append`, then remove the inline tag. If phase-blocker count is high, flag it.
