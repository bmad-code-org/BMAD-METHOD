...
2. **The requirement source at this altitude.** The container's own Requirements section holds it: the source's lines as stable ids, each mapping to a parent id in `covers`. A referenced numbered source replaces it; a `spec-<slug>/SPEC.md` directly inside the container is that source. When `bmad-spec` is needed, invoke it with the container-local output folder so its spec and companions land beside the container; `covers` still maps the spec's ids upward. The architecture spine, UX design, and research inform everything below either way.
