...
Then tell the user the entries are ready to build, and what that means for how they build. With `bmad-build`, each ticket is refined during the build: the builder questions the user and writes the criteria into its plan, so refining here repeats that work. With `bmad-build-auto`, a loop, or a factory, nobody answers questions during the build and the entry is all the builder gets: recommend one more review of the sequence and of each entry now, and offer the checkpoints where none are set.

## Pulling and refining a ticket

Pull with `uv run {skill-root}/scripts/tickets.py --project-root {project-root} pull <epic folder> <n>`: it writes `<type>-<nn>-<slug>.md` from the entry in a fixed layout, `status: draft`, `refined: false`; the templates apply only to tickets you write. A later change to anything the entry holds goes to the entry and the file together, and through the store once published; where their `blocked_by` differ, the script uses the file's and `status` marks the row `drift`.
...
Refining follows `{workflow.refinement}`. For an epic's stories — the next one by default, all when the user asks — it is a review of their entries: read each entry, its source, the finished siblings and their build records; improve description, `verify`, references, and notes with the user; adjust order and blockers, and run the set check on the revised draft when they change.
