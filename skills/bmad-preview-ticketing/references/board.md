...
An unrefined ticket may be published for planning visibility. It keeps `refined: false`; backlog means available to start. With a tracker, the `refined: false` line stays in the published body. Refinement updates that same file and remote item, preserving identity.
...
- A ticket waiting on a person or an answer, not on a blocker: set `blocked_at` (date) and `blocked_reason`; clear both when it moves. `next` lists it under `blocked`.
...
{output_folder}/
  {active_initiative}/                        # example active_initiative=initiative-checkout
    ...
    epic-cart-rules/
      ...
      spec-cart-rules/
      story-01-cart-service-scaffold.md        # pulled
