...
Keep every value quoted. `--date` accepts canonical `YYYY-MM-DD HH:MM` and legacy `MM-DD-YYYY HH:MM` input, then writes the canonical ISO form. Unpadded spellings like `1-2-2026 9:05` are accepted and normalized to the padded ISO form, but a value that does not parse is rejected with `ok: false`, `restored: true` and exit 1, before the file is touched, and the whole update is a no-op. So pass `{date}` in either accepted form, or omit the flag entirely and let the script stamp the current time itself. That format carries a space, which is why the flag must be quoted: unquoted, `--date 07-28-2026 14:23` splits into two argv words and dies at argparse (`{"ok": false, "error": "argument error: unrecognized arguments: 14:23"}`, exit 2). `--file` and `--ref` are quoted for the same reason — an `{{ config.implementation_artifacts }}` path containing a space breaks them exactly the same way.
