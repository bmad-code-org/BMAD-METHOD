...
1. **Find the review target.** Check in this order and stop at the first that identifies it:
   - **Request or recent conversation.** A PR (resolve via `gh pr view`), commit, branch, commit range, staged or uncommitted changes, a provided diff or file list, or a spec file. A spec sets `spec_file`; its frontmatter `baseline_commit`, if present, is the diff baseline — a spec without one does not identify the diff.
