...
5. Resolve config: `uv run {project-root}/_bmad/scripts/resolve_config.py --project-root {project-root} --key core.output_folder --key modules.bmm.active_initiative`. `{date}` is the current system datetime. `{slug}` is what the brief is about, in kebab-case: the run lands in `brief-{slug}/brief-{slug}.md`.
