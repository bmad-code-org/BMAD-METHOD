...
1. Explicit argument
   ...
   - If it points to a file whose frontmatter `type` is `story`, `spike`, or `bug`, it is a ticket, not a spec, whatever its `status`: ingest it as starting intent and proceed to INSTRUCTIONS. Never set `spec_file` to it and never write to it.
