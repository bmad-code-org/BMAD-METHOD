...
| `help/ticketing-and-epics.md` | How `bmad-preview-ticketing` works (initiatives, epic inception, thin stories), why it differs from epics plus sprint planning, and which to pick. |
...
- Bigger than one session: does the user already have enough to say or paste (an idea they can explain in detail, notes, intent.md, single ticket, a transcript, a brief, a PRD)?
  - Yes → `bmad-spec`, then have it break the spec into stories, then one `bmad-build` per story.
...
| "Can BMad build my stories by itself?" | `bmad-build-auto`, dispatched per story by a loop | It suits settled decisions and well specified stories, with someone reading the results (`help/unattended-builds.md`). |
...
| `bmad-spec` | The hub. Distills any input into the contract builds read, updates it, and breaks it into an ordered story list. It does not coach. | `{output_folder}/specs/spec-{slug}/` with `SPEC.md`, companions, `stories.yaml` |
...
| `bmad-preview-ticketing` | Preview. A ticket tree run as a board: initiatives, epics, thin stories refined when pulled, one-off bugs, optional tracker. | Ticket files under `{output_folder}/{active_initiative}/` and `{output_folder}/backlog/` |
...
| `bmad-spec` | Its open questions and assumptions, if any. Then story breakdown and `bmad-build`, or straight to `bmad-build` when one session can do it. |
