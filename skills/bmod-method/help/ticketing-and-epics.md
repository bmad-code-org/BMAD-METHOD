...
- **Build from the entry.** An entry needs no file to be built. `bmad-build` reads the entry and its epic, plus the story file when a person refined it, and plans the story's acceptance criteria from the epic's Requirements and Done when, the entry's description, and its `Verify:` check. Its plan, `<type>-<slug>-plan.md`, sits beside `tickets.toml` and carries the ticket's status. After completed work changes the picture, the remaining breakdown is revisited.
- **Refining is optional.** With `bmad-build`, a story is refined during the build: the builder questions the user and writes the criteria itself. Refining in ticketing pulls the story's file if it has none (`<type>-<slug>.md`, written by a script; from then on the file is truth and the entry keeps only the id, type, title, prerequisites, and `hitl`), then reviews the file with the user — description, check, references, notes, order, prerequisites — and writes no Given/When/Then. Recommend that review when the epic will run unattended (`bmad-build-auto`, a loop, a factory), because nobody answers questions during the build. Full acceptance criteria are written in ticketing only for a ticket with no epic, a bug, or when the user asks.
...
- **The board.** "What's next?" shows what is ready to refine (only the tickets that need it), ready to start, in progress, or blocked (waiting on a person or an answer). Asked about the initiative, it covers every epic. A prerequisite in `after` can be a story in the same epic, a story in another epic, or a whole epic.
- **Status.** A ticket's `status` lives in its plan and belongs to the build, which moves it as far as `built`, meaning the build finished, and never further. The user marks a ticket done, or an orchestrator does; told so, the ticketing skill runs `tickets.py mark` for them. Ticketing also writes `dropped` when the user says so. On a tracker store the tracker's status sits in the ticket file as `tracker_status`, so a card moved on the board never makes the build skip planning.
- **Other abilities.** A one-off bug or story goes straight into `backlog/` with no epic. Tickets can be published to a tracker such as Jira, Linear, or GitHub, by default the whole breakdown at inception; publishing writes each ticket's file, and the markdown file stays the working copy. On the repo store, committing the approved `tickets.toml` is the publish.
...
- **Files only where they earn it.** A story is an entry until someone refines it or publishes it to a tracker; the build's plan is the only file every story gets.
...
While ticketing is in preview, `bmad-create-epics-and-stories` with `bmad-sprint-planning` remains the supported route for planning and building. The retrospective reads only the ticket tree, so an epics-route project has none until `bmad migrate method` moves it onto the tree.

- **Unattended loops.** bmad-loop reads `sprint-status.yaml` or an existing `stories.yaml`. It does not read `tickets.toml` yet.
...
- Has a PRD and an architecture, and wants criteria written up front for every story → the epics route.
