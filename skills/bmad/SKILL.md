...
If the user explicitly asks to set up, update, repair or doctor this BMad installation, or to see its status, load `references/setup.md` and follow it. Otherwise use the read-only help process below. Missing BMad project files or scripts never turn an ordinary help request into setup.
