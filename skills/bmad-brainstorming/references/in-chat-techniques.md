...
The library is large — never pull it whole into context. The only way in is the helper, always passing `--file {workflow.brain_methods}`. Subcommands of `uv run --no-project {skill-root}/scripts/brain.py --file {workflow.brain_methods}`:
