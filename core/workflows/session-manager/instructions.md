...
5. Initialize session state:
   ...
   project: "{project}"
   tokens:
     max: 150000        # Default token budget (fallback if missing)
     initial: 0
     current: 0
     peak: 0            # Highest token usage during session
     saved: 0           # Tokens saved via isolation
   agents_spawned: []
   artifacts: []
   milestones: []
   context_summary: ""
...
Token Savings from Isolation: {saved}
```
...
# Token tracking
tokens:
  max: 150000              # Token budget limit (default: 150000)
  initial: 0
  current: 45000
  peak: 52000              # Highest token usage during session
  saved: 145000            # Tokens saved via isolation
...
## Token Deserialization and Defaults

When reading session files, implementations MUST handle missing token fields gracefully:

```yaml
# Canonical token fields (unified schema)
tokens:
  max: 150000              # Token budget limit; default if missing
  initial: 0               # Starting token count; default: 0
  current: 0               # Current token usage; default: 0
  peak: 0                  # Highest usage during session; default: 0
  saved: 0                 # Tokens saved via isolation; default: 0
```

**Fallback Logic:**
1. If `tokens.max` is missing or null, default to `150000`
2. If any other token field is missing, default to `0`
3. **Migration:** If legacy `tokens.saved_by_isolation` exists, read it as `tokens.saved`
4. When serializing outputs (resume, status, tokens), read from `tokens.max` for the `{max}` placeholder
5. Calculate `{percentage}` as `(tokens.current / tokens.max) * 100`
6. Calculate `{saved}` from `tokens.saved` for savings displays

**Warning Threshold:**
- When `tokens.current / tokens.max > 0.80` (80%), display yellow warning indicator
- When `tokens.current / tokens.max > 0.95` (95%), display red critical indicator
