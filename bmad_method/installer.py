...
def format_toml_value(value) -> str:
    """Format a scalar/list as a TOML literal.

    TOML basic strings and JSON strings escape the same way, so json.dumps
    matches the Node installer's TOML output byte-for-byte. ensure_ascii=False is
    essential; the default would \\uXXXX-escape non-ASCII (emoji, em dashes) and
    break parity.
    None is special-cased (JSON emits null, we need "").
    """
    if value is None:
        return '""'
